 ...
 * return a replacement (which may be the unchanged url). Note: This is not a useful
 * thing to do, at least within the Tenon API project.
 *
 * The configuration property inlineContent (default true) specifies whether to perform
 * the core function of inlining local javascript and css. If falsey, this module simply
 * fulfills with the content of the HTML file.
 *
 * Returns a promise. It resolves with a string containing HTML with inlined JS and CSS.
 * (or just the untouched HTML if inlineContent is set to false.)
 */
...
module.exports = function(conf) {
  return new Promise(function(resolve, reject) {
    ...
    fs.readFile(conf.fname, {encoding: 'utf8'}, function(err, data) {
      ...
      if (data) {
        filecontent = data;
        if (!conf.inlineContent) {
          //console.log('returning non-inlined');
          resolve(filecontent);
          return;
        }
        files = [];
